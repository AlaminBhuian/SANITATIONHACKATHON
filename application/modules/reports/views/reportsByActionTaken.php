...
<table width="98%" border="0" cellpadding="5" cellspacing="0" bordercolor="#F2F2F2" class="rpt-table" align="center">
  <tr bgcolor="#DEE1E7" align="right">
    <th width="19%" align="center" bgcolor="#EEEEE6"><strong>Action</strong></th>    
 ...
 <tr class="<?php echo $alt ?>">
     <td align="left" style="text-transform: capitalize;"><a <?php if($item['totalComplaint']>0){?>href="<?php echo site_url('reports/getDetailsByActionTaken/'.$item['status_id'].'/'.$dateFrom.'/'.$dateTO);}?>" target="blank"><?php echo $item['name']; ?></a></td>     
     <td align="right"><a <?php if($item['totalComplaint']>0){?>href="<?php echo site_url('reports/getDetailsByActionTaken/'.$item['status_id'].'/'.$dateFrom.'/'.$dateTO);}?>" target="blank"><?php echo $item['totalComplaint']; $total += $item['totalComplaint']; ?></a></td>
        <td align="right" class="last"><?php echo round($k =(($item['totalComplaint']*100)/$totaldata),2);$m += $k; ?>%</td>
  </tr>
      <?php }?>   
