...
<table width="98%" border="0" cellpadding="5" cellspacing="0" bordercolor="#F2F2F2" class="rpt-table" align="center">
  <tr bgcolor="#DEE1E7" align="right">
    <th width="19%" align="center" bgcolor="#EEEEE6"><strong>Delivery Method</strong></th>
  ...
  $total =0;
  $m =0;
  foreach($typeOFDelevery as $item){
     if (($i % 2) == 0)
    {
        $alt = "odd";
    } else
 ...
 <tr class="<?php echo $alt ?>">
     <td align="left" style="text-transform: capitalize;"><a <?php if($item['total_method']>0){?>href="<?php echo site_url('reports/getDetailsByDelivaryMethod/'.$item['id'].'/'.$dateFrom.'/'.$dateTO);}?>" target="blank"><?php echo $item['varName']; ?></a></td>
     <td align="right"><a <?php if($item['total_method']>0){?>href="<?php echo site_url('reports/getDetailsByDelivaryMethod/'.$item['id'].'/'.$dateFrom.'/'.$dateTO);}?>" target="blank"><?php echo $item['total_method']; $total += $item['total_method']; ?></a></td>
        <td align="right" class="last"><?php echo round($k = (($item['total_method']*100)/$totalDelivery),2); $m += $k; ?>%</td>
  </tr>
      <?php }?>   
